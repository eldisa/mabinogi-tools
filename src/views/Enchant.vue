                                ...
                                <el-form-item label="武器類型">
                                    <div class="flex gap-2">
                                        ...
                                        <el-select v-model="quickWeaponType" style="width: 160px">
                                            <el-option
                                                v-for="w in weaponsInCategory"
                                                :key="w.label"
                                                :label="w.display ?? w.label"
                                                :value="w.label"
                                            />
                                        </el-select>
                                    </div>
...
const weaponCategory = ref<WeaponCategory>("魔法");
const quickWeaponType = ref<string>("魔杖");

const weaponsInCategory = computed(() => QUICK_WEAPON_OPTIONS.filter((w) => w.category === weaponCategory.value));
// 切換大類時，若目前武器不屬於該類則跳到該類第一個
watch(weaponCategory, () => {
    const list = weaponsInCategory.value;
    if (!list.some((w) => w.label === quickWeaponType.value)) {
        quickWeaponType.value = list[0]?.label ?? "";
    }
});
...
type WeaponCategory = "近距離" | "遠距離" | "魔法" | "音樂";

interface WeaponOpt {
    label: string; // 內部 key（同時為能力對照表的鍵）
    display?: string; // 下拉顯示名稱，預設 = label
...
const WEAPON_CATEGORIES = [...new Set(QUICK_WEAPON_OPTIONS.map((w) => w.category))];
...
interface SlotDef {
    ...
    isWeapon?: boolean;
    isOffhand?: boolean;
    exclusive?: boolean; // true = 只吃符合 limit 的賦予，不含中性（無限制）賦予
    limits: string[];
}
...
const quickViewData = computed((): QuickViewRow[] => {
    ...
    return QUICK_VIEW_SLOTS.filter((slot) => !(hideRobe && slot.label === "袍/翅膀"))
        ...
        .map((slot) => {
            ...
            let applicable = enchants.filter((e) =>
                slot.exclusive
                    ? e.limit.some((l) => limits.includes(l))
                    : isNoRestriction(e) || e.limit.some((l) => limits.includes(l)),
            );
