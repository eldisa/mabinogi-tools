...
watch([targetStatKey, targetMinValue], () => { successCount.value = 0; });
// Reset min value when switching stat
watch(targetStatKey, () => { targetMinValue.value = 1; });
</script>
        ...
        <div class="max-w-2xl mx-auto">
            <header class="mb-8 text-center pt-8 pb-4">
                <div class="flex justify-center mb-3">
                    <img
                        :src="`${baseUrl}itemImage/5040566.png`" alt="聖水"
            ...
            <el-card class="mb-4 bg-gray-800 border-2 border-accent/30 shadow-lg rounded-xl">
                ...
                <div v-if="targetStat && !isFixed(targetStat)" class="flex items-center gap-3 mb-4">
                    ...
                    <el-input-number
                        ...
                        :min="1" :max="targetStat.max" :step="1" :precision="0"
            ...
            <el-card class="mb-4 bg-gray-800 border-2 border-accent/30 shadow-lg rounded-xl">
                <div class="mb-4 border-b border-gray-700 pb-3 flex items-center gap-3 flex-wrap">
                    ...
                    <template v-if="totalUses > 0">
                        <span class="text-sm text-gray-400 ml-auto">
                            已用 <span class="text-white font-bold">{{ totalUses.toLocaleString() }}</span> 次
                        </span>
                        ...
                        <span v-if="costPerUse > 0" class="text-sm text-gray-400">
                            · <span class="text-yellow-400 font-bold">{{ (totalUses * costPerUse).toLocaleString() }}</span> 金
                        </span>
                    </template>
                </div>
                ...
                <div class="mb-5 flex items-center gap-3">
                    ...
                    <el-input-number
                        v-model="costPerUse" :min="0" :step="10000" :precision="0"
                    ...
                    <div v-if="lastResult" :key="totalUses">
                        ...
                        >
                            ...
                            <div class="text-3xl font-bold"
                                :class="lastResult.isSuccess ? 'text-green-400' : hasTarget ? 'text-orange-400' : 'text-blue-400'">
                ...
                <template v-if="showHistory">
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                <tr v-for="r in history" :key="r.useId"
                                ...
                                >
                                    ...
                                    <td class="py-1.5 pr-4"
                                        :class="hasTarget && r.stat.key === targetStatKey
                                            ? r.isSuccess ? 'text-green-400 font-semibold' : 'text-orange-400'
                                            : 'text-gray-300'">
                                        {{ r.stat.label }}
                                    </td>
                                    <td class="text-center py-1.5 px-3"
                                        :class="hasTarget && r.isSuccess ? 'text-green-400 font-semibold' : 'text-gray-300'">
...
.result-tile {
    ...
    padding: 1.2rem 1.4rem; display: flex; flex-direction: column; gap: 2px;
    transition: border-color 0.15s, background 0.15s;
}
.result-tile--hit  { border-color: #22c55e; background: rgba(5, 46, 22, 0.4); }
.result-tile--miss { border-color: #f97316; background: rgba(67, 20, 7, 0.3); }
